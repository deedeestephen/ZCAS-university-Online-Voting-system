...
- **SMS Notifications**: Once a decision is made, an automated SMS is sent to the student (via Twilio) informing them that their voting profile is either approved or rejected.

### 3. Election Execution
- **Election Scheduling**: Admins can set the exact start and end times for the election.
- **Start Notifications**: Admins can blast SMS notifications to all verified students announcing that the election is open.
- **Casting a Vote**: Once the election begins and a student is approved, they can log in, view candidates categorized by position (e.g., President, Vice President), and cast their ballot.
...
- **Live Turnout**: Admins can view the live voter turnout percentage and totals across different faculties (Business, Law, IT, Finance).
...
### 5. Role Management (Super Admin)
- The initialized Super Admin can manage administrative access, delegating partial permissions (such as 'verifier' or 'editor' roles) to other staff members to distribute the workload without compromising top-level configuration.
...
- **Increased Participation**: With an easy-to-use interface accessible from any mobile or desktop browser—coupled with proactive SMS reminders—student turnout is expected to increase significantly.
...
- **Backend/API (Full-stack Mode)**: Node.js, Express
- **Database & Authentication**: Firebase Auth, Cloud Firestore (with comprehensive Zero-Trust Security Rules)
- **External Communications**: Twilio SDK for automated transactional SMS
...
3. A **Twilio Account** configured with an active phone number, Account SID, and Auth Token.

### Step 1: Environment Variables
Create a `.env` file on your host server based on `.env.example`. You must populate the following:
```env
...
# Vite/Firebase variables are handled via firebase-applet-config.json
# or standard Firebase web client initialization.
```
...
Start the production server. This kicks off the Express backend which serves the built React frontend and securely handles the Twilio API routes.
```bash
npm start
```

### Step 5: Database Security Rules Deployment
Ensure that the `firestore.rules` file is properly deployed to your Firebase project to secure all incoming and outgoing data. You can do this using the Firebase CLI:
```bash
npm i -g firebase-tools
firebase deploy --only firestore:rules
```

### Note on Containerization (Docker)
Because the project uses standard `npm run build` and `npm start` commands producing a standard Node.js server, it can be easily containerized using a `Dockerfile`:
```dockerfile
FROM node:18-alpine
WORKDIR /app
COPY package*.json ./
